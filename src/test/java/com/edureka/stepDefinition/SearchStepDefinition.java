...
/**
 * Step Definition class for Search Module
 * Handles all Cucumber steps related to search functionality
 */
public class SearchStepDefinition extends AllFunctionality {
	
	// Base class for WebDriver access
    private final Base base;
    
 // Used to store first search result for validation across steps
    private String firstResultTitle;

    /**
     * Constructor Injection (Dependency Injection using PicoContainer)
     */
    ...
    public void user_is_ready_on_homepage_for_search_module() {
        Assert.assertTrue(Pages.get().shp.isSearchTriggerDisplayed(), "Search bar not visible");
    }

    
    @Then("search bar should be visible for search module")
    public void search_bar_should_be_visible_for_search_module() {
        Assert.assertTrue(Pages.get().shp.isSearchTriggerDisplayed(), "Search bar is not visible");
    }
    ...
    public void search_panel_should_open_for_search_module() {
        Assert.assertTrue(Pages.get().shp.isSearchPanelDisplayed(), "Search panel did not open");
    }
    ...
    public void relevant_search_results_should_be_displayed_for_search_module() {
        Assert.assertTrue(Pages.get().srp.areResultsDisplayed(), "Results not displayed");
    }
    ...
    public void user_searches_using_datatable_for_search_module(DataTable table) {
        ...
        for (int i = 1; i < data.size(); i++) {
            ...
            Pages.get().shp.clickSearchTrigger();
            Assert.assertTrue(Pages.get().shp.isSearchPanelDisplayed(),
                    "Search panel did not open for keyword: " + keyword);

            Pages.get().shp.enterKeyword(keyword);
            Pages.get().shp.pressEnter();

            Assert.assertTrue(Pages.get().srp.areResultsDisplayed(), "Search failed for keyword: " + keyword);
        }
    }
    ...
    public void default_search_page_should_open_for_search_module() {
        Assert.assertTrue(base.getDriver().getCurrentUrl().contains("search"), "Default search page did not open");
    }

  
    @Then("system should not crash for search module")
    public void system_should_not_crash_for_search_module() {
        Assert.assertNotNull(base.getDriver().getTitle(), "System crashed or title is null");
    }

    
    @Then("callback form should be displayed for search module")
    public void callback_form_should_be_displayed_for_search_module() {
        Assert.assertTrue(Pages.get().srp.isCallbackFormDisplayed(), "Callback form is not displayed");
    }

   
    @When("user enters valid mobile number from excel sheet {string} row {int} for search module")
    public void user_enters_valid_mobile_number_from_excel_sheet_row_for_search_module(String sheet, Integer row) {
        init(sheet);
    ...
    public void user_enters_invalid_mobile_number_from_excel_sheet_row_for_search_module(String sheet, Integer row) {
        init(sheet);
        String mobile = getData(row, 6);
    ...
    public void system_should_accept_valid_10_digit_mobile_number_for_search_module() {
        String mobile = Pages.get().srp.getEnteredMobileNumber();

        Assert.assertEquals(mobile.length(), 10, "Valid mobile number was not accepted properly");
        Assert.assertTrue(mobile.matches("\\d{10}"), "Valid mobile number is not a 10 digit number");
    }

    /**
     * Step: Then system should not accept more than 10 digits for search module
     * Asserts that the system does not accept more than 10 digits.
     */
    ...
    public void system_should_not_accept_more_than_10_digits_for_search_module() {
        ...
        Assert.assertEquals(mobile.length(), 10, "BUG: System accepted more than 10 digits. Actual value: " + mobile);
    }

  
    @When("user enters course name {string} in callback form")
    public void user_enters_course_name_in_callback_form(String courseName) {
        Pages.get().srp.enterCourseLookingFor(courseName);
    }

   
    @When("user clicks Get a call back button")
    public void user_clicks_get_a_call_back_button() {
        Pages.get().srp.clickGetCallBackButton();
    }

        @Then("callback request should be submitted successfully")
    public void callback_request_should_be_submitted_successfully() {
        Assert.assertTrue(Pages.get().srp.isCallBackSuccessMessageDisplayed(),
                "Callback request was not submitted successfully");
    }

   
    @Then("callback request should not be submitted for invalid mobile")
    public void callback_request_should_not_be_submitted_for_invalid_mobile() {
        boolean isSubmitted = Pages.get().srp.isCallBackSuccessMessageDisplayed();

        if (isSubmitted) {
            Assert.fail("BUG: Callback request submitted even for invalid mobile number");
        }
    }
    ...
    public void user_should_be_navigated_to_search_results_page() {
        ...
        Assert.assertTrue(url.contains("/search"), "User not navigated to search results page");
    }

    /**
     * Step: When user selects popular keyword from excel sheet {string} row {int}
     * Selects a popular keyword from the specified Excel sheet and row.
    ...
    public void verify_keyword_match() {
        Assert.assertTrue(Pages.get().srp.areResultsDisplayed(), "Search results not displayed");
    }
    ...
    public void verify_category_navigation() {
        Assert.assertTrue(Pages.get().srp.getCategoryHeading().length() > 0, "Category page not loaded");
    }
    ...
    public void search_results_should_match_selected_keyword(String keyword) {
        ...
        Assert.assertTrue(url.contains(keyword.toLowerCase().replace(" ", "-")) || url.contains(keyword.toLowerCase()),
    ...
    public void user_should_be_navigated_to_category_page_for(String category) {
        String url = base.getDriver().getCurrentUrl().toLowerCase();
        String formatted = category.toLowerCase().replace(" & ", " ").replace("&", "").replace(" ", "-").trim();

        Assert.assertTrue(url.contains(formatted.split("-")[0]), "Navigation to category page failed");

        Assert.assertTrue(
                Pages.get().srp.getCategoryHeading().toLowerCase().contains(category.toLowerCase().split(" ")[0]),
   ...
   //Submit callback request
    ...
    public void filtered_results_should_be_displayed_for(String role) {
        Assert.assertTrue(Pages.get().srp.areResultsDisplayed(), "Filtered results not displayed");

        Assert.assertTrue(Pages.get().srp.isJobRoleSelected(role), "Filter not applied correctly");
    }
}